...
COPY backend/new-guides ./backend/new-guides
COPY backend/databases/AppSchool.db /app/AppSchool.seed.db
...
# Volume is root-owned; seed as root then drop to skateboard for the app.
USER root
CMD ["sh", "-c", "mkdir -p /app/backend/databases && ([ -s /app/backend/databases/AppSchool.db ] || cp /app/AppSchool.seed.db /app/backend/databases/AppSchool.db) && chown -R skateboard:skateboard /app/backend/databases && exec su skateboard -s /bin/sh -c 'exec skateboard-backend'"]
