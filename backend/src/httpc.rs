...
//! `stripe.rs` are private to that module, so AppSchool needs its own client
//! to mint xAI ephemeral voice tokens. Zero-crate: libcurl is linked as a
//! *system library* (`#[link(name = "curl")]`), exactly as `stripe.rs` links
//! it, and every `unsafe` block below carries a `// SAFETY:` justification.
//! The exported surface ([`get`], [`post_json`], [`post_form`], [`HttpResponse`])
//! is safe.
//!
...
//! complete body, which is all the token mint needs and keeps the module small.
