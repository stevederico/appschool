...
describe('ALLOWLIST completeness', { skip: !IS_TEMPLATE }, () => {
  ...
  it('every backend boilerplate file is allowlisted', () => {
    ...
    // course content, and the JS seed that loads new-guides/ / new-reps/ into SQLite.
    const missing = walkRepo(join(REPO, 'backend'))
      .filter(f => !f.includes('/databases/') && !f.includes('/new-guides/') && !f.includes('/new-reps/') && !f.includes('/seed/') && !f.includes('/public/') && !f.endsWith('config.json') && !f.includes('.env') && !f.endsWith('.DS_Store') && !f.endsWith('PRODUCTION.md') && !f.endsWith('httpc.rs'))
